...
def train():
    ...
    train_dataset = train_dataset.shuffle(100).map(preprocess).batch(
        32).repeat()
        ...
        for epoch in range(start_epoch, 120):
            try:
                for batch, data in enumerate(train_dataset):
                    ...
                    if batch % 10 == 0:
                        logging.info(
                            'Epoch: {}, iter: {}, loss: {}, train_acc: {}'.
                            format(epoch, batch, train_loss.result(),
                                   train_accuracy.result()))
            except KeyboardInterrupt:
                logging.info('interrupted.')
                model.save_weights(ckpt_path.format(epoch=epoch))
                logging.info('model saved into: {}'.format(
                    ckpt_path.format(epoch=epoch)))
                exit(0)
